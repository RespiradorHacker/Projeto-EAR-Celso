 ...
 * ======================================================================================================================  
 *  
 * Notes on HW and SW updates:
 * ===========================
 * - This project has a very tight schedule so some documents are not being updated as they should
 * - As this firmware has to be updated due to firmware improvement, we'll use it to keep HW info updated as well
 * - The HW related docs will be updated but not at the same time as this document.
 *  ===============================
 *  UPDATED Arduino Pro Mini Pinout
 *  ===============================
 *  TXO - TxD for Bootloader and FlexLed Serial Monitor (@57K6bps)
 *  RXI - RxD for Bootloader and FlexLed Serial Monitor
 *  RST - /Reset
 *  GND - GND
 *  D2 - reserved
 *  D3 - Latch 74HC165 Inputs for SPI Based Inputs expander. Level 0 for loading 1 for shifting IN
 *  D4 - Piezo self-osc Buzzer OUT - Active High
 *  D5 - BDC Motor PWM Hi-Side A - Provides 1 KHz PWM for Hi-side motor full bridge - Active High (CHANGED)
 *  D6 - BDC Motor PWM Hi-Side B - Provides 1 KHz PWM for Hi-side motor full bridge - Active High
 *  D7 - Multipurpose Red Led - Active High
 *  D8 - GPIO Motor Low Side A - Active High
 *  D9 - GPIO Motor Low Side B - Active High (CHANGED)
 *  D10 - Latch 74HC595 Outputs for SPI SPI Based Outputs expander. Rising edge transfer shift regiters to outputs
 *  D11 - MOSI for SPI based IO expander
 *  D12 - MISO for SPI based IO expander
 *  D13 - SCK for SPI based IO expander
 *  A0 - Analog readings for Pot1 - TIDAL VOLUME
 *  A1 - Analog readings for Pot2 - BPM (Breaths per Minute)
 *  A2 - Analog readings for Pot3 - Overpressure Threshold 
 *  A3 - Analog readings for Pot4 - Assisted Pressure Threshold (available only on Phase 2)
 *  A4 - Compression Arm Position Sensor (CHANGED)
 *  A5 - reserved (CHANGED)
 *  A6 - Analog readings of Pressure Sensor - MPX5010 or similar 10 KPa range Px sensor (0-5V output)
 *  A7 - Motor Shunt Current - 0.43V/A 
 *    - on mosfet Driver current will be limited to around 5A (2.15V)
 *    - on relay driver current will not be limited but it will be sensed until around 8 Amps, where it'll be clamped
 *  VCC - +5V provided by external +5V system voltage regulator
 *  RST - /Reset
 *  GND - GND
 *  RAW - Not connected
 *  
 *  SPI Based IO - IO1 Module - mandatory  - given names matches ULN2803 (outputs) and 74HC165 (inputs) nomenclature 
 *  
 *  EXP IO - INPUTS   
 *  ===============
 *  Inputs A-H are pulled-up, they are activated by connecting to GND, use NPN sensors or NO to GND.
 *  
 *  INPUT A - reserved for definition of Configuration (0) or Operation (1)
 *  INPUT B - reserved
 *  INPUT C - I/E ratio LSB \_ (Dip switch possible) CHANGED 00= 1:4  01= 1:3  10= 1:2 11= 1:1 (0= input opened 1= input GND) 
 *  INPUT D - I/E retio MSB / 
 *  
 *  INPUT E - T_Hold LSB \_ (Dip switch possible) CHANGED  00= 150ms  01= 250ms  10= 500ms 11= 1s  (0= input opened 1= input GND)
 *  INPUT F - T_Hold MSB /  
 *  
 *  INPUT G - 
 *  INPUT H - 
 ...
 *  EXP IO - OUTPUTS   
 *  ================
 *  ULN2803 outputs 1-8 are NPN type, you can connect directly 12V coil, T73 or auxiliary relays (e.g. to switch 24V or AC loads). 
 *  Simply connect the coil on +12V (same as system +12V PSU) and selected output. 
 *  Keep current to maximum 250mA per output and do NOT use 24V load directly because the ULN2803 diode pullups will connect the 24V load to PSU +12V
 *  
 *  OUTPUT 1 - reserved (not connected on ULN2003)
 *  OUTPUT 2 - 12V Automotive Siren for alarms
 *  OUTPUT 3 - 
 *  OUTPUT 4 - 
 *  OUTPUT 5 -
 *  OUTPUT 6 -
 ...
 *  Notes on hardware:
 *  - the AMBU should be the one with the largest volume available. By using motor reversal, we can control AMBU compression displacement (Tidal Volume) on software
 *  - the BDC motor used for AMBU compression is a Mabuchi car window lift motor, left or right side
 *  - the Arm position sensor is a car tank float level sensor. Parameters are necessary:
 *    - EOC_UP (End Of Course Ambu Uncompressed)
 *    - EOC_Down (End of Course Ambu compressed at maximum)
 *    - recommended use of retundant normally open EOC, hall or switch type, slightly after EOC parameter limits 
 *  - there are 2 versions of motor drive; relays or mosfets
 *    - relay motor driver version uses T73 relays, available everywhere. Notes below: 
 *      - Use only LSA e LSB signals when using relays, PWM outputs HSA and HSB will NOT work
 *      - Inspiration speed will depend only on external voltage applied. Use a 5A (minimum) bench PSU with 47.000uF capacitor so PSU will not fail on motor start
 *      - Note that relay coil voltage should be obtained from CPU supply, before +5V regulator
 *    - mosfet drivers uses N-channel low level drive mosfets, or N-CH and P-CH mosfets, depending on version
 *      - N-CH mosfets should have Vgs of 2V or lower, Vds of 40V minimum and at least 15A continuos current. 
 *      - P-CH mosfets should have Vds of 40V minimum and at least 15A continuos current. 
 *    - The potentiometers are linear 10K type (1K-22K range are acceptable)
 *  
...
Apr 9, 2020 - First funcional prerelease FW02 - Tidal Volume and BPM functional
Pressure limits not present

Apr 10, 2020 - Improved documentation
- included HW descripton on this firmware and definition of latest update.

Apr 10, 2020 - FIO2 controle removed from project scope.
It was removed because FIO2 Control could be independent of eAR system.
Even if we keep the concept of using 2 valves to control enriched air mix, 
it could be one FIO2 controller for several eAR systems.
So we can use an Arduino dedicated controller and 2 valves for all.
Just to register the concept:
There are 2 pneumatic valves: O2 and air valves
- O2 is always opened
- Air is cycled in 20-200 ms range every second, depending on potentiometer setpoint
- Both valves are stopped if intake bag is full. Bag full EOC signals that.
A calibration procedure is necessary to measure O2 and Air intake.
It could be simple as measure time to fill 2 separate bags. 
The bags are near atmosferic pressure, vinyl type if possible.
Example: 30 liter O2 bag and modulated 3 liter air bag fill at same time 
it would turn out to be 10:1 ratio or 91% oxygen

#R200410EAR01 Implement pressure limit based on OverPx Threshold Pot
- Adds motor cutoff in inspiration phase only
  - expiration was excluded because there could be a peak on exhaust valve opening
- Beep on OverPx created
- So now Overpressure behaviour could be described as:
  - On overpressure motor stops immediately
  - when motor stops, beeps starts and beep ends on start of next breathing cycle
- Operator then should do:
  - reduce Tidal volume or
  - reduce BPM (dP/dT will change) or
  - raise Overpressure limit
- results were repeatable and 1cm H20 resolution was accomplished

#R200411EAR01 Implements external Watchdog Timer
- WDT implemented as a simple oscillator connected to /RESET
- Latch_Inputs signal is used as output to watchdog timer
- This signal generates 50 Hz and acts on a charge pump that charges a capacitor
- the capacitor charged is the same as the oscillator and if charged makes it stop oscillating
- Letch_Inputs needs to be stable only when latching inputs and shifting data
  - there is no collateral effect on using Latch_Inputs on 50 Hz (except some low level EMI generation)
- The physical connection recommended is on last IO EXP unit.
  - There is a last connector that connects to watchdog and provides a loopback MOSI - MISO connection
    - this loopback is to ensure data has shifted correctly on SPI shift registers allowing check of MOSI, MISO and SCK lines
    - 74HC595 and 74HC165 shift registers are tested too.

NOTICE: To use Arduino Bootloader itis necessary to REMOVE watchdog and RECONNECT it to Run

#R200413EAR01 - Creates calibration procedure targeted at 400ml Tidal Volume Set precision
  - introduced new float sensor from Gol G3
  - readings of new float sensor are decreasing resistance with course
  - creation of new procedure to implement both curves, with positive and negative inclination
  - documentation of calibration process
  - at the end, 400ml will be precise, all the rest Tidal Volumes are aproximations

#R200413EAR02 - Implements options of IE (1:1 to 1:4) and T_Hold (150ms to 1s)
IE_Ratio=1:1, 1:2, 1:3 or 1:4
Plateau_Hold_Time=150ms, 250ms, 500ms or 1s

#R200423EAR01 - Arm position and angle changed 
- Applies to G3 model only
- Set new values

#R200424EAR01 - Motor State Machine altered in order to avoid deadlock in inspiration process

#R200424EAR02 - Added Hardware Debugging Info - Testing Expansion IO

#R200428EAR01 - Added New Pressure Sensor MPS20N0040D for testing purposes
  - Pressure Sensor is available everywhere, it's too simple, just the mems wheatstone bridge resistors
  - No reliable data sheet available - not homologated
  - created new pressure sensor Hardware based on low cost instrumentation amplifier
    - Offset and gain adjustment allows 2 point calibration
  - No temperature compensation, undetermined dP on dTemp
  - creates #defines for MPX5010 and MPS20N0040D

#R200508EAR01 - First new electronics test
  - changes in High Side Motor Bridge Hardware
    - Prototypes were built with High Side P-channel mosfets
    - Products were built with High Side N-channel mosfets
    - So PWM polarity is inverted


Hardware Debugging:
Testing Expansion IO:
  - Best if powered on a bench supply to observe current abnormalities
  - provide Loopback connector on Expanded IO 1-9 2-10, ..., 8-16
  - uncomment #define SPI_Loopback_Mode to set it
  - On FlexLed O0 and I0 must present always the same values when Loopbacked - check all 256 values
  - If something goes wrong, check signals when they departure from CPU
    - The series resistors avoid short circuits so you can scope them

TO BE Implemented:
- Hybrid (HW+SW) Overcurrent protection
- Assisted breathing 
- Alarms
...
// #R200508EAR01 - First new electronics test
// #define HS_P_Mosfets // Prototype HS Mosfets are P-Channel
#define HS_N_Mosfets // Product HS Mosfets are N-Channel

// Red_Led is used for process/task monitoring when below line is uncommented
#define Debug_Task_Period TRUE
#define Task_Period_Set 2 // Defines task under period measurement (0-9) do not left undefined, it will generate an error msg
#if (Task_Period_Set>20)
  #define Task_Common TRUE  //
#endif

#define Beep_On_OverPx TRUE // #R200410EAR01 - Enables Beep on overpressure

//#define SPI_Loopback_Mode 1 // SPI Loopback Mode is set for IO Expansion Testing - 
...
// #R200428EAR01 - Added New Pressure Sensor MPS20N0040D for testing purposes
#define Px_Sensor_MPX5010 TRUE
//#define Px_Sensor_MPS20N0040D TUE

#define Ticks 10 // define o tempo entre cada execucao de loop, deve ser tunado para refletir degradacao do periodo em funcao da execucao das tasks

#define STD_PWM_Duty_Up 40 // percentage used for testing purposes
#define STD_PWM_Duty_Down 50 // percentage used for testing purposes

// ====================================
//    Arm Position Sensor Definition
// ====================================
// #R200413EAR01 - New float sensor from Gol G3
// - readings of new float sensor are decreasing resistance with course
// - creation of new procedure to implement both cases
//
// In order to define a new position sensor and calibrate it you must follow this sequence of steps
// 1 - Create an empty sensor definition set as the examples below
// 2 - Check with a DVM if your position sensor increases or decreases the resistance level when moving the Arm down
//    - if the sensor increases resistante (as Corsa fuel tank float Sensor) do not define (use comments on definition) Complement_Arm_Readings
//    - if the sensor decreases resistante (as Gol G3 fuel tank float sensor) define Complement_Arm_Readings TRUE
// 3 - Set EOC_UP to 0 EOC_Down to 511
// 4 - Disconnect eAR motor, Compile and Run eAR FW
// 5 - Use an external power supply with a low voltage e.g. 3V applied directly to the motor
// 6 - Move motor to Arm UP position, check on FlexLed AS:XXX, register the EOC_UP position
// 7 - Invert motor connections, move to a position that corresponds to full stroke tidal volume (aprox 800ml)
//     register the EOC_Down position
// 8 - Fill the sensor definition with the EOC_UP and EOC_Down. Tidal_400ml_Pos should remain undefined
// 9 - Recompile and run, still with motor unattached.
// 10 - Attach motor using the same polarity definitions as the last one. The Arm Sensor now should be functional
// 11 - Adjust Tidal volume pot so the REAL measured Tidal volume equals to 400 ml. This is THE TIDAL calibration point
// 12 - read Arm Position Sensor and define Tidal_400ml_Pos with this value.
// 13 - recompile and run 
// 

// Arm Sensor Definition
#define GolG3_Arm_Sensor TRUE
//#define Corsa_Arm_Sensor TRUE

// Specific sensor Data
#ifdef Corsa_Arm_Sensor
  // #define Complement_Arm_Readings TRUE
  #ifdef Complement_Arm_Readings
  #else
    ...
    // #define Tidal_400ml_Pos 251
  #endif
#endif

// Specific Sensor Data
/*
#ifdef GolG3_Arm_Sensor
  #define Complement_Arm_Readings TRUE
  #ifdef Complement_Arm_Readings
    #define EOC_UP 137 // #R200423EAR01 - Arm position and angle changed - Old value 275    // #R200406EAR01 defines End Of Course Ambu Uncompressed
    #define EOC_Down 230 // #R200423EAR01 - Arm position and angle changed - Old value 357  // #R200406EAR01 desfines End of Course Ambu Smashed
    //#define Tidal_400ml_Pos 304
  #else
  #endif
#endif
*/
  ...
  #define Complement_Arm_Readings TRUE
  #ifdef Complement_Arm_Readings
    #define EOC_UP 200 // #R200423EAR01 - Arm position and angle changed - Old value 275    // #R200406EAR01 defines End Of Course Ambu Uncompressed
    #define EOC_Down 285 // #R200423EAR01 - Arm position and angle changed - Old value 357  // #R200406EAR01 desfines End of Course Ambu Smashed
    //#define Tidal_400ml_Pos 304
  #else
  #endif
#endif

  #define Raw_Arm_Course EOC_Down-EOC_UP  // defines Course
  // #define Tidal_400ml_Pos 
  
// =======================================  
//  END of Arm Position Sensor Definition
// =======================================
...
#define PxMaxLimit_Min 15 // #R200410EAR01 Implement pressure limit based on Pot_Limit_PxMax 
#define PxMaxLimit_Max 40 // #R200410EAR01 Implement pressure limit based on Pot_Limit_PxMax
...
// #define FlexLed_Debug TRUE  // focus on specific items for debugging purpose
  ...
  #define sDump_Pot_Tidal TRUE // Pots   
  //#define sDump_Pot_BPM TRUE  
  //#define sDump_Pot_Limit_PxMax TRUE  
  //#define sDump_Pos_Pot4 TRUE  
  ...
  #define sDump_PH TRUE   // Plateau Hold Time SET
...
#define Pot_Limit_PxMax Pos_Pot3  // #R200410EAR01 
...
U8 WDT; // #R200411EAR01 necessary for  Watchdog Timer Oscillator control
...
U16 PWM_Duty; // #R200405EAR01 varia de 0-255 
...
#define ErrPot_Limit_PxMax 0x04
...
// #R200508EAR01 created because of changes in Hardware
// Prototypes were built with His=gh Side P-channel mosfets
// Product were built with High Side N-channel mosfets
// So PWM polarity is inverted
...
void Set_PWM_HSA (void)
{
  ...
  // On prototype HS mosfets are P-Channel 
  #ifdef HS_P_Mosfets
     Real_Duty=HSA_Duty; 
  #endif

  // All mosfets are N-Channel
  #ifdef HS_N_Mosfets
      Real_Duty=255-HSA_Duty;
  #endif
  
  analogWrite(HSA,Real_Duty);
}
...
void Set_PWM_HSB (void)
{
  ...
  // All mosfets are N-Channel
  #ifdef HS_N_Mosfets
      Real_Duty=255-HSB_Duty;
  #endif
  
  analogWrite(HSB,Real_Duty);
}
...
void controller()
{
  ...
  // #R200411EAR01 Implements external Watchdog Timer
  // WDT Signal Generation
  
  WDT++;  // Increment WDT Variable

  // Latch_Inputs oscillates precisely @ 50 Hz
  // There is a small glitch on signal but it doesn't affect anything
  if (WDT&0x01)  digitalWrite(Latch_Inputs, HIGH);
  else digitalWrite(Latch_Inputs, LOW);
  ...
  // #R200408EAR01 - Added pressure sensor on SW
  // #R200428EAR01 - Added New Pressure Sensor MPS20N0040D for testing purposes
  #ifdef Px_Sensor_MPX5010
    Px_Sensor=(Average_Read(Px_Sns) + 1) / 2;
    MeasuredPressure=Px_Sensor/5; 
  #endif

  #ifdef Px_Sensor_MPS20N0040D
    // 9-bit treatment: 0 cm H2O reads 250
    // For direct readings in cm H2O you have to divide dy 2 
    Px_Sensor=(Average_Read(Px_Sns) + 1) / 2;
    Dummy16=(Px_Sensor-250)/2;
    MeasuredPressure=Dummy16; 
  #endif

  // #R200406EAR01 - adds motor absolute position based on car tank level sensor
  // #R200413EAR01 - creation of new procedure to implement both cases

  ArmPosSensor = (Average_Read(ArmPosSns) + 1) / 2;
  
#ifdef Complement_Arm_Readings
  ArmPosSensor = 511- ArmPosSensor; // complement sensor readings if necessary 
#endif
  ...
  // Controls Motor STOP
  // ===================
  // Motor must be stopped in one of the following conditions:
  // - End of Course (EOC) reached, Up or Down
  // - Max pressure reached only in inspiration phase #R200410EAR01
  //    - expiration was excluded because there could be a peak on exhaust valve opening
  // 
  switch (Motor_Spin)
    {
    ...
    case M_Spin_CW:
      // Inspiration: Compressing AMBU
      // if (ArmPosSensor >= EOC_Down) Motor_Stop();
      if (ArmPosSensor >= Final_Arm_Position)
        {
        // normal stop
        Motor_Stop();
        }
      else if (MeasuredPressure>PressureMaxLimit)     // #R200410EAR01 Implement pressure limit based on OverPx Threshold Pot
        {
        // stopped by Overpressure
        Motor_Stop();
        
#ifdef Beep_On_OverPx
        Beep_On(); 
#endif        
        }

      // check uncorrect position
      if (ArmPosSensor < EOC_UP-50)
        {
        // Sensor Failure
        // ArmSensorFailure++;
        IncErrorCount();
    ...
    case M_CW_Ending:
      if (Timer_Motor)
        ...
        Motor_CW(PWM_Duty);
        // #R200424EAR01 - Motor State Machine altered in order to avoid deadlock in inspiration process
        // duplicated from above, with Timer_Motor running
        
        if (!Timer_Motor)
          {
          // Timer_Motor está zerado, mudanca de fase
          Motor_Status = M_CW_Stop; //
          Motor_Stop();
          Timer_Motor = Plateau_Hold_Time;  //
          }
...
// #R200413EAR02 - Implements options of IE (1:1 to 1:4) and T_Hold (150ms to 2s)
Dummy= ((InputsExp[0]& 0b00001100))>>2;
switch (Dummy)
  {
  case 0:
    IE_Ratio=4;  // 
...
Dummy= ((InputsExp[0]& 0b00110000))>>4;
switch (Dummy)
  {
  case 0:
    Plateau_Hold_Time=15;  // *10ms 150ms
  ...
  case 1:
    Plateau_Hold_Time=25;  // *10ms 250ms
    break;
    
  case 2:
    Plateau_Hold_Time=50;  // *10ms 500ms
    break;
    
  case 3:
    Plateau_Hold_Time=100;  // *10ms 1s
    break;
  }
  ...
  case 2:
    ...
    // ==========================================================
    // #R200408EAR08 - Implements Final_Arm_Position as f(Course, TidalVolumeSet)
    // #R200413EAR01 - Creates calibration procedure targeted at 400ml Tidal Volume Set
#ifdef Tidal_400ml_Pos
    // calibrated course with 400ml calibration
    Final_Arm_Position=Tidal_400ml_Pos-EOC_UP;  // 400ml displacement
    Final_Arm_Position=Final_Arm_Position*2;    // aprox 800ml displacement
    Dummy16=TidalVolumeSet/10;                  // converts TVS to 20-80 range
    Final_Arm_Position=((Final_Arm_Position)*Dummy16)/100;  // calculates displacement using pot as percentage
    ...
    Final_Arm_Position=((Raw_Arm_Course)*Dummy16)/100; // calculates displacement using pot as percentage
    Final_Arm_Position=Final_Arm_Position+EOC_UP;       // adds initial position to get Final_Arm_Position
#endif
    break;
  ...
  case 3:
    ...
    // #R200410EAR01 PressureMaxLimit calculation based on pot
    // calculates PressureMaxLimit according to POT
    // 20% 102 on pot
    // 80% 409 on pot
    // divide by 10 pot result will give a 10-40 cm H2O range
    // 

    PressureMaxLimit=(Pot_Limit_PxMax/10);

    if (PressureMaxLimit<PxMaxLimit_Min-5) // PM > MAX was not considered as error due to desperate operator behaviour
      {
      // possible error on Pot or human operator
      IncErrorCount();
      ErrorID=ErrPot_Limit_PxMax;
      PressureMaxLimit=Last_Valid_PML; // assumes last valid reading
      }
    else
      {
      // boundaries check
      if (PressureMaxLimit<PxMaxLimit_Min) PressureMaxLimit=PxMaxLimit_Min ;
      else if (PressureMaxLimit>PxMaxLimit_Max) PressureMaxLimit=PxMaxLimit_Max ;
          else
            {
            // looks like a valid reading
            Last_Valid_PML=PressureMaxLimit;
            }
      } 
...
#ifdef sDump_Pot_Tidal    
    Serial.print(" P1:");
    Serial.print(Pot_Tidal, DEC);
#endif

#ifdef sDump_Pot_BPM
    Serial.print(" P2:");
    Serial.print(Pot_BPM, DEC);
#endif

#ifdef sDump_Pot_Limit_PxMax
    Serial.print(" P3:");
    Serial.print(Pot_Limit_PxMax, DEC);
#endif

#ifdef sDump_Pos_Pot4
    Serial.print(" P4:");
...
// ADD Here Plateau_Hold_Time
#ifdef sDump_PH // Plateau Hold Time SET
    Serial.print(" PH:");
    Serial.print(Plateau_Hold_Time, DEC);
    Serial.print("0");
#endif
...
// Add here AV Assisted Vaccum Trigger
    Serial.println();
